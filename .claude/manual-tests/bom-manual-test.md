...
| TC-CREATE-05 | Duplicate of unique_together set | First create a BOM (Product=SKU-4001, type=ebom, version=A, revision=01) — note: seed already has this | 1. Create new BOM with the same Product+Type+Version+Revision combo | — | Form-level error under **Revision** field: "A EBOM vA.01 already exists for SKU-4001 in this tenant. Bump the revision or version." NOT a 500. (Fixed via `BillOfMaterialsForm.clean()` — see [.claude/tasks/bom_sqa_fixes_todo.md](../tasks/bom_sqa_fixes_todo.md).) | | |
...
| TC-CREATE-10 | Effective_to before effective_from | On create form | 1. Name: `Date Test`<br>2. Effective from: `2026-04-30`<br>3. Effective to: `2026-04-25`<br>4. Submit | — | Form error under **Effective to**: "Effective-to date cannot be earlier than effective-from." (Fixed in `BillOfMaterialsForm.clean()`.) | | |
...
| TC-DELETE-04 | Delete Approved BOM blocked | Approved BOM (SKU-4002 SBOM) | 1. POST to `/bom/boms/<approved-pk>/delete/` directly via DevTools | — | Red toast: "Only Draft or Under Review BOMs can be deleted. Approved or Released BOMs must be marked Obsolete instead." Redirected to detail. BOM still exists. (View tightened to match the UI — only `is_editable()` BOMs can be deleted.) | | |
...
| TC-NEG-03 | Negative quantity | Add line with quantity `-1` | 1. Submit | `-1` | Form error under **Quantity**: "Ensure this value is greater than or equal to 0.0001." Zero is also rejected. | | |
| TC-NEG-04 | Scrap_percent > 100 | Add line with scrap=`150` | 1. Submit | `150` | Form error under **Scrap percent**: "Ensure this value is less than or equal to 100." Negative scrap is also rejected. | | |
...
| TC-NEG-15 | Snapshot with deleted component | Create a BOM, capture revision, delete the component product (or mark obsolete), rollback | 1. Click Rollback on the captured revision | — | Lines whose component SKU no longer matches an existing product are skipped, **but** a yellow warning toast lists the count and missing SKUs (e.g. "3 line(s) skipped — components no longer in catalog: SKU-X, SKU-Y, SKU-Z."). The change_summary on the new rollback revision also records the same. | | |
